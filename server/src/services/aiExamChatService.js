...
const resolveProviderConfig = () => {
  ...
  if (openAiKey) {
    const provider = isXaiKey(openAiKey) ? "xai" : "openai";
    const baseURL = explicitBaseUrl || (provider === "xai" ? "https://api.x.ai/v1" : undefined);
    const defaultModel = provider === "xai" ? "grok-3-mini" : "gpt-4o-mini";

    return {
      provider,
      apiKey: openAiKey,
      baseURL,
      model: toSafeText(process.env.OPENAI_MODEL).trim() || (provider === "xai" ? toSafeText(process.env.XAI_MODEL).trim() : "") || defaultModel,
    };
  }
